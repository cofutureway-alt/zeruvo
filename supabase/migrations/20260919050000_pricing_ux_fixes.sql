-- ============================================================
-- PRICING UX FIXES:
-- 1) models_public_view also exposes the BASE prices (the admin table and
--    pricing modal read them; the effective prices stay for the catalog).
-- 2) usage_multiplier may be 0 — "free for plans" (0 weighted tokens).
-- ============================================================

-- 0 = free on weighted-token plans (was >= 1)
alter table public.models drop constraint if exists models_usage_multiplier_check;
alter table public.models
  add constraint models_usage_multiplier_nonneg check (usage_multiplier >= 0);

-- new columns change the view's column list → full rebuild
