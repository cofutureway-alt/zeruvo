...
/**
 * Client-side announcements layer:
 *  - marquee bars render fixed above everything
 *  - popups render centered once per session (dismiss persisted locally)
 *  - audience targeting resolved from the auth session + active subscription
 */
export function AnnouncementsLayer({ locale, pathname }: { locale: string; pathname: string }) {
	...
	useEffect(() => {
		void (async () => {
			...
			setPopup(eligible.find((a) => a.type === 'popup') ?? null);
		})();
	}, [pathname]);
