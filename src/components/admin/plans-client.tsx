...
import { Plus, Pencil, Package, Check } from 'lucide-react';
...
export function PlansClient() {
	...
	const load = useCallback(async () => {
		...
		const [{ data: plansData }, { data: modelsData }] = await Promise.all([
			admin.from('plans').select('*, plan_models(model_id)').order('price_usd'),
			admin
				.from('models')
				.select('id,upstream_model_id,enabled_for_users')
		...
		setPlans(
			(plansData ?? []).map((p) => ({
				...p,
				model_ids:
					p.plan_models?.map((pm: { model_id: string }) => pm.model_id) ?? [],
		...
		<div className="space-y-6">
			<header className="flex items-center justify-between">
				<div>
					...
					<p className="mt-0.5 text-sm text-[var(--nx-muted)]">
						Subscription tiers with daily weighted-token allowances.
					</p>
				</div>
				<button
			...
			<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
				{plans.map((p) => (
					<article
						key={p.id}
						className="rounded-xl border border-[var(--nx-border)] bg-[var(--nx-surface)] p-5"
					>
						<div className="flex items-start justify-between">
							...
							<div className="flex shrink-0 gap-1.5">
								{p.default_free && (
									<span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[11px] text-emerald-400">default free</span>
								)}
								{!p.active && (
									<span className="rounded-full bg-zinc-700/40 px-2 py-0.5 text-[11px] text-zinc-400">inactive</span>
								)}
							</div>
						</div>
						...
						<p className="mt-2 text-sm tabular-nums text-[var(--nx-muted)]">
							{Number(p.daily_weighted_tokens).toLocaleString()} weighted tokens / day
						</p>
						<p className="mt-1 text-xs text-[var(--nx-muted)]">{p.model_ids.length} models included</p>
						<button
							onClick={() => setEditing(p)}
							className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg border border-[var(--nx-border)] py-2 text-sm hover:border-indigo-500/50 hover:text-indigo-300"
						>
							<Pencil size={14} />
							Edit
						</button>
...
function PlanEditor(props: {
	...
	const [unit, setUnit] = useState(p?.duration_unit ?? 'days');
	...
	const [selected, setSelected] = useState<Set<string>>(new Set(p?.model_ids ?? []));
		...
		<div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/60 p-4 backdrop-blur-sm">
			<div className="w-full max-w-2xl rounded-2xl border border-[var(--nx-border)] bg-[var(--nx-surface)] shadow-2xl">
				...
				<div className="max-h-[70vh] space-y-5 overflow-y-auto p-6">
					{/* names */}
					<div>
						<p className="mb-2 text-sm font-medium">Name & description per locale</p>
							...
							{LOCALES.map((l) => (
								<div key={l} className="space-y-1.5">
									<input
										...
										placeholder={`Name (${l})${l === 'en' ? ' — required' : ''}`}
									...
									<input
										...
										className="w-full rounded-lg border border-[var(--nx-border)] bg-transparent px-3 py-2 text-xs outline-none focus:border-indigo-500"
									/>
								</div>
							))}
						</div>
					...
					<div className="grid grid-cols-2 gap-3">
						<label className="block">
							...
							<input
								...
								className="mt-1 w-full rounded-lg border border-[var(--nx-border)] bg-transparent px-3 py-2 text-sm tabular-nums outline-none focus:border-indigo-500"
							/>
						</label>
						<label className="block">
							...
							<input
								...
								className="mt-1 w-full rounded-lg border border-[var(--nx-border)] bg-transparent px-3 py-2 text-sm tabular-nums outline-none focus:border-indigo-500 disabled:opacity-40"
							/>
						</label>
						<label className="block">
					...
					<div>
						<p className="mb-2 text-sm font-medium">
							Included models <span className="font-normal text-[var(--nx-muted)]">({selected.size})</span>
						</p>
						<div className="max-h-44 space-y-1 overflow-y-auto rounded-xl border border-[var(--nx-border)] p-2">
							{props.models.map((m) => (
								<button
									...
									className="flex w-full items-center gap-2 rounded-lg px-3 py-1.5 text-start text-sm hover:bg-indigo-500/5"
								>
									<span
