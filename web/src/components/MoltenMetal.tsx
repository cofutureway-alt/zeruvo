...
interface MoltenMetalProps {
	...
	opacity?: number;
	/** Upper bound on devicePixelRatio. Lower it for large decorative fills. */
	maxDpr?: number;
	className?: string;
}
...
const colorModeToFloat = (mode: MoltenMetalColorMode) => (mode === 'ember' ? 1 : mode === 'frost' ? 2 : 0);
...
const ctxMap = new WeakMap<HTMLElement, MoltenCtx>();
...
export default function MoltenMetal({
	...
	useEffect(() => {
		...
		try {
			...
			// No WebGL2 — leave the section on its CSS background.
			return;
		}
		...
		// The canvas is pointer-events:none so hero buttons stay clickable —
		// track on window and map into canvas space ourselves.
		...
		if (reduce) {
			// One frame at a fixed offset: a still of the same field, no RAF.
	...
	useEffect(() => {
		...
		([color1, color2, color3] as const).forEach((hex, idx) => {
			const rgb = hexToRgb(hex);
			const target = u[`uColor${idx + 1}`].value as Float32Array;
			target[0] = rgb[0];
			target[1] = rgb[1];
			target[2] = rgb[2];
		});
